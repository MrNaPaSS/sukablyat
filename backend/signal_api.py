...
app = Flask(__name__)
CORS(app, origins=['https://app.nomoneynohoney.online', 'http://localhost:5173', 'http://127.0.0.1:5173'], 
...
TOP3_CACHE_FILE = os.path.join(ROOT_DIR, 'top3_signals_cache.json')
...
def save_feedback_to_stats(user_id, signal_id, feedback, pair=None, direction=None, confidence=None):
    ...
    # Нормализуем feedback для обратной совместимости
    if feedback == 'failure':
        feedback = 'failed'
        feedback_record['feedback'] = 'failed'  # Обновляем в записи тоже
...
# ===== АКТИВНЫЕ СИГНАЛЫ ПОЛЬЗОВАТЕЛЕЙ =====
ACTIVE_SIGNALS_FILE = os.path.join(ROOT_DIR, 'active_signals.json')

def load_active_signals():
    """Загрузка активных сигналов пользователей"""
    try:
        if os.path.exists(ACTIVE_SIGNALS_FILE):
            with open(ACTIVE_SIGNALS_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        return {'active_signals': {}, 'last_updated': datetime.now().isoformat()}
    except Exception as e:
        print(f'[ERROR] Ошибка загрузки активных сигналов: {e}')
        return {'active_signals': {}, 'last_updated': datetime.now().isoformat()}

def save_active_signals(data):
    """Сохранение активных сигналов"""
    try:
        with open(ACTIVE_SIGNALS_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f'[ERROR] Ошибка сохранения активных сигналов: {e}')

def load_top3_cache():
    """Загрузка кеша ТОП-3 сигналов"""
    try:
        if os.path.exists(TOP3_CACHE_FILE):
            with open(TOP3_CACHE_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        return {
            'last_updated': None,
            'forex_signals': [],
        ...
        print(f'[ERROR] Ошибка загрузки кеша ТОП-3: {e}')
        return {
            'last_updated': None,
            'forex_signals': [],
            'otc_signals': [],
...
def save_top3_cache(data):
    """Сохранение кеша ТОП-3 сигналов"""
    try:
        with open(TOP3_CACHE_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f'[ERROR] Ошибка сохранения кеша ТОП-3: {e}')

async def generate_top3_signals_auto():
    """Автоматическая генерация ТОП-3 сигналов для всех пользователей"""
    try:
        print(f'[AUTO-TOP3] Начинаем автоматическую генерацию ТОП-3...')
        
        # Проверяем расписание рынка
        from market_schedule import MarketSchedule
        market_schedule = MarketSchedule()
        ...
        print(f'[AUTO-TOP3] Статус рынка: open={is_open}, forex_available={forex_available}')
        
        cache_data = {
            'last_updated': datetime.now().isoformat(),
            'forex_signals': [],
            'otc_signals': [],
            'next_generation': (datetime.now() + timedelta(minutes=10)).isoformat()
        }
        
        # Генерируем Forex сигналы если рынок открыт
        if is_open and forex_available:
            try:
                print(f'[AUTO-TOP3] Генерируем Forex ТОП-3...')
                forex_signals = await forex_generator.get_best_signals(limit=3)
                if forex_signals:
                    for signal in forex_signals:
                        cache_data['forex_signals'].append({
                            'signal_id': f'auto_forex_{signal.pair.replace("/", "_")}_{int(datetime.now().timestamp())}',
                            'pair': signal.pair,
                            'type': signal.direction,
                            'direction': signal.direction,
                            'entry': signal.entry_price,
                            ...
                            'signal_type': 'forex',
                            'timestamp': datetime.now().isoformat(),
                            'reasoning': getattr(signal, 'reasoning', 'Автоматический анализ')
                        })
                    print(f'[AUTO-TOP3] Сгенерировано {len(cache_data["forex_signals"])} Forex сигналов')
            except Exception as e:
                print(f'[AUTO-TOP3] Ошибка генерации Forex: {e}')
        
        # Генерируем OTC сигналы (всегда доступны)
        try:
            print(f'[AUTO-TOP3] Генерируем OTC ТОП-3...')
            otc_pairs = ['EUR/USD (OTC)', 'NZD/USD (OTC)', 'USD/CHF (OTC)']
            for pair in otc_pairs:
                if len(cache_data['otc_signals']) >= 3:
                    break
                signal = await otc_generator.generate_otc_signal(pair)
                if signal and signal.confidence >= 0.60:
                    cache_data['otc_signals'].append({
                        'signal_id': f'auto_otc_{pair.replace("/", "_").replace(" ", "_")}_{int(datetime.now().timestamp())}',
                        'pair': pair,
                        'type': signal.direction,
                        'direction': signal.direction,
                        'entry': str(signal.entry_price),
            ...
            print(f'[AUTO-TOP3] Сгенерировано {len(cache_data["otc_signals"])} OTC сигналов')
        except Exception as e:
            print(f'[AUTO-TOP3] Ошибка генерации OTC: {e}')
        
        # Сохраняем кеш
        save_top3_cache(cache_data)
        print(f'[AUTO-TOP3] Кеш сохранен. Следующая генерация: {cache_data["next_generation"]}')
        
    except Exception as e:
        print(f'[AUTO-TOP3] Критическая ошибка: {e}')
        import traceback
        traceback.print_exc()

def auto_generation_worker():
    """Фоновая задача для автоматической генерации ТОП-3 каждые 10 минут"""
    print(f'[AUTO-TOP3] Запуск фоновой задачи генерации ТОП-3...')
    
    while True:
        try:
            # Запускаем генерацию
            asyncio.run(generate_top3_signals_auto())
            
            # Ждем 10 минут
            time.sleep(10 * 60)  # 10 минут
            
        except Exception as e:
            print(f'[AUTO-TOP3] Ошибка в фоновой задаче: {e}')
            time.sleep(60)  # Ждем минуту при ошибке

def set_user_active_signal(user_id, signal_data):
    """Установка активного сигнала для пользователя"""
    try:
        data = load_active_signals()
        current_time = datetime.now().isoformat()
        
        data['active_signals'][str(user_id)] = {
            'signal_id': signal_data.get('signal_id'),
            'pair': signal_data.get('pair'),
            'direction': signal_data.get('direction'),
            'confidence': signal_data.get('confidence'),
            'entry_price': signal_data.get('entry_price'),
            'expiration': signal_data.get('expiration'),
            'start_time': signal_data.get('start_time'),
            'created_at': current_time,
            'status': 'active'
        }
        data['last_updated'] = current_time
        
        save_active_signals(data)
        print(f'[ACTIVE_SIGNAL] Установлен активный сигнал для пользователя {user_id}')
        return True
    except Exception as e:
        print(f'[ERROR] Ошибка установки активного сигнала: {e}')
        return False

def get_user_active_signal(user_id):
    """Получение активного сигнала пользователя"""
    try:
        data = load_active_signals()
        return data['active_signals'].get(str(user_id))
    except Exception as e:
        print(f'[ERROR] Ошибка получения активного сигнала: {e}')
        return None

def clear_user_active_signal(user_id):
    """Очистка активного сигнала пользователя"""
    try:
        data = load_active_signals()
        if str(user_id) in data['active_signals']:
            del data['active_signals'][str(user_id)]
            data['last_updated'] = datetime.now().isoformat()
            save_active_signals(data)
            print(f'[ACTIVE_SIGNAL] Очищен активный сигнал для пользователя {user_id}')
        return True
    except Exception as e:
        print(f'[ERROR] Ошибка очистки активного сигнала: {e}')
        return False

@app.route('/api/signal/activate', methods=['POST'])
def activate_signal():
    """Активация сигнала пользователем (серверная привязка)"""
    try:
        data = request.get_json()
        user_id = data.get('user_id')
        ...
        if not user_id or not signal_data:
            return jsonify({
                'success': False,
                'error': 'user_id and signal required'
            }), 400
        
        # Проверяем, нет ли уже активного сигнала
        existing_signal = get_user_active_signal(user_id)
        if existing_signal:
            return jsonify({
                'success': False,
                'error': 'User already has active signal',
                'active_signal': existing_signal
            }), 409
        
        # Устанавливаем активный сигнал
        if set_user_active_signal(user_id, signal_data):
            return jsonify({
                'success': True,
            ...
            return jsonify({
                'success': False,
                'error': 'Failed to activate signal'
            }), 500
            
    except Exception as e:
        print(f'[ERROR] Ошибка активации сигнала: {e}')
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/signal/active', methods=['GET'])
def get_active_signal():
    ...
    try:
        ...
        active_signal = get_user_active_signal(user_id)
        if active_signal:
            return jsonify({
                'success': True,
...
@app.route('/api/signal/complete', methods=['POST'])
def complete_signal():
    """Завершение активного сигнала (после фидбека)"""
    try:
        data = request.get_json()
        user_id = data.get('user_id')
        feedback = data.get('feedback')  # 'success' или 'failure'
        ...
        # Получаем активный сигнал
        active_signal = get_user_active_signal(user_id)
        if not active_signal:
            return jsonify({
                'success': False,
                'error': 'No active signal found'
            }), 404
        
        # Сохраняем фидбек
        if feedback:
            save_feedback_to_stats(
                user_id, 
                active_signal['signal_id'], 
                feedback,
                active_signal['pair'],
                active_signal['direction'],
                active_signal['confidence']
            )
            
            # Обновляем статистику
            signal_type = 'forex' if 'forex' in active_signal['signal_id'] else 'otc'
            user_stats = update_user_stats(user_id, signal_type, feedback)
        
        # Очищаем активный сигнал
        clear_user_active_signal(user_id)
        
        return jsonify({
            'success': True,
            'message': 'Signal completed successfully',
            'user_stats': user_stats if feedback else None
        })
        
    except Exception as e:
        print(f'[ERROR] Ошибка завершения сигнала: {e}')
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@app.route('/api/signal/top3/latest', methods=['GET'])
def get_latest_top3():
    """Получение последних ТОП-3 сигналов из кеша"""
    try:
        cache = load_top3_cache()
        
        if not cache.get('last_updated'):
            return jsonify({
                'success': False,
                'error': 'No cached signals available',
                'message': 'ТОП-3 сигналы еще не сгенерированы'
            }), 404
        
        return jsonify({
            'success': True,
            'signals': {
                'forex': cache.get('forex_signals', []),
                'otc': cache.get('otc_signals', [])
            },
            'last_updated': cache.get('last_updated'),
            'next_generation': cache.get('next_generation')
        })
        
    except Exception as e:
        print(f'[ERROR] Ошибка получения ТОП-3 из кеша: {e}')
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/signal/top3/status', methods=['GET'])
def get_top3_status():
    """Получение статуса автоматической генерации ТОП-3"""
    try:
        cache = load_top3_cache()
        ...
        if cache.get('next_generation'):
            next_generation = datetime.fromisoformat(cache['next_generation'])
        ...
        return jsonify({
            ...
            'has_signals': bool(cache.get('forex_signals') or cache.get('otc_signals'))
        })
        
    except Exception as e:
        print(f'[ERROR] Ошибка получения статуса ТОП-3: {e}')
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def generate_signal_analysis(prompt):
    """Генерация анализа сигнала на основе промпта с вариативностью - 50 ВАРИАНТОВ"""
    import random
    ...
    # РАСШИРЕННЫЕ СЛОВАРИ ДЛЯ МАКСИМАЛЬНОЙ ВАРИАТИВНОСТИ
    time_factors = [
    ...
    if is_success:
        # 20 ВАРИАНТОВ УСПЕШНЫХ СДЕЛОК
        success_analyses = [
            f"✅ АНАЛИЗ УСПЕШНОЙ СДЕЛКИ {pair}: 1️⃣ Трейдер правильно выбрал направление {direction} и следовал стратегии. 2️⃣ Ключевые факторы: точный анализ рынка, {random_emotion} при входе, правильная оценка условий. 3️⃣ Рекомендации: найти оптимальную точку входа, продолжать стратегию, масштабировать успех. 💪 Отлично! Продолжай в том же духе! Зарабатывай еще больше!",
            f"✅ ПРОФЕССИОНАЛЬНЫЙ АНАЛИЗ {pair}: 1️⃣ Трейдер показал дисциплину в следовании {direction} сигналу {random_time}. 2️⃣ Успех достигнут благодаря: правильному таймингу, анализу тренда, управлению рисками в условиях {random_condition}. 3️⃣ Следующие шаги: улучшить точность входа, развивать навыки анализа, увеличивать объемы. 💪 Превосходно! Твой подход работает! Продолжай зарабатывать!",
            f"✅ МАСТЕРСКИЙ АНАЛИЗ {pair}: 1️⃣ Трейдер продемонстрировал профессионализм в {direction} операции. 2️⃣ Факторы успеха: глубокий анализ, правильный выбор момента, {random_emotion} в условиях {random_condition}. 3️⃣ Развитие: изучать новые паттерны, оптимизировать стратегию, расширять горизонты. 💪 Блестяще! Ты на правильном пути! Зарабатывай стабильно!",
            f"✅ ЭКСПЕРТНЫЙ РАЗБОР {pair}: 1️⃣ Трейдер проявил мастерство в {direction} сделке {random_time}. 2️⃣ Успех обеспечен: качественным анализом, {random_emotion}, адаптацией к {random_condition}. 3️⃣ Планы: углубить знания, расширить стратегии, увеличить прибыльность. 💪 Потрясающе! Ты настоящий профессионал! Зарабатывай больше!",
            f"✅ ТОП-АНАЛИЗ {pair}: 1️⃣ Трейдер блестяще выполнил {direction} операцию в условиях {random_condition}. 2️⃣ Ключ к успеху: {random_emotion}, точный расчет, правильный тайминг {random_time}. 3️⃣ Перспективы: масштабировать успех, изучать новые рынки, повышать доходность. 💪 Фантастика! Ты на вершине! Продолжай доминировать!",
            f"✅ БЕЗУПРЕЧНЫЙ РЕЗУЛЬТАТ {pair}: 1️⃣ Трейдер виртуозно использовал {direction} сигнал, проявив {random_emotion}. 2️⃣ Успех: идеальный тайминг {random_time}, подтверждение по {random_key_metrics}, грамотное управление в условиях {random_condition}. 3️⃣ Перспективы: автоматизировать лучшие паттерны, закрепить {random_analysis_style} анализ. 👑 Фантастика! Ты доминируешь на рынке! Повторяй этот успех!",
            f"✅ СТРАТЕГИЧЕСКАЯ ПОБЕДА {pair}: 1️⃣ Трейдер не только выбрал {direction}, но и верно определил цели. 2️⃣ Причина: четкий {random_analysis_style} анализ, подтвержденный {random_key_metrics}, и {random_emotion}. 3️⃣ Совет: документировать условия входа и выхода. 🎯 Глубокий подход! Твоя стратегия дала плоды!",
            f"✅ ЧЁТКИЙ ПРОГНОЗ {pair}: 1️⃣ Великолепное исполнение {direction} сигнала {random_time}. 2️⃣ Использованы: грамотный стоп-лосс, тейк-профит и уверенность в {random_condition}. 3️⃣ Шаги вперед: протестировать эту тактику на других парах. ✨ Прекрасная работа! Дисциплина — залог прибыли!",
            f"✅ АЛЬФА-ТРЕЙД {pair}: 1️⃣ Трейдер поймал движение {direction}, как настоящий охотник, с {random_emotion}. 2️⃣ Факторы: идеальный вход на подтверждении {random_key_metrics} в условиях {random_condition}. 3️⃣ Задание: сохранить спокойствие и повторять системные действия. 🏆 Ты гений торговли! Рынок тебе подвластен!",
            f"✅ СИСТЕМНЫЙ УСПЕХ {pair}: 1️⃣ Трейдер показал высокий уровень {random_emotion} и дисциплины. 2️⃣ Подтверждение {direction} сигнала: тщательный анализ {random_key_metrics} и терпеливое ожидание {random_time}. 3️⃣ Развитие: продолжать вести дневник и фокусироваться на **процессе**. 📈 Это не удача, это система!",
            f"✅ СНАЙПЕРСКАЯ ТОЧНОСТЬ {pair}: 1️⃣ Идеальный {direction} вход {random_time}. Трейдер поймал пик движения. 2️⃣ Причина: высокий уровень {random_emotion}, точное определение {random_key_metrics} и анализ {random_condition}. 3️⃣ Совет: сохранить эту четкость! Масштабировать эту стратегию на другие активы. 🎯 Браво! Ты настоящий снайпер рынка!",
            f"✅ ТРЕЙД-ЛЕГЕНДА {pair}: 1️⃣ Чистейшее исполнение {direction} сигнала по плану, несмотря на {random_condition}. 2️⃣ Факторы: **железная дисциплина**, подтверждение по {random_analysis_style} анализу. 3️⃣ Дальнейшие шаги: зафиксировать этот успех в дневнике и разобрать условия входа/выхода. 🌟 Поздравляю! Ты создал свою легенду.",
            f"✅ МАСТЕР РИСК-МЕНЕДЖМЕНТА {pair}: 1️⃣ Не только верное {direction} направление, но и грамотный расчет риска/прибыли. 2️⃣ Успех обеспечен: **оптимальный** размер позиции, **правильный** стоп-лосс, {random_emotion}. 3️⃣ Фокус: не менять правила! Четко следовать плану входа {random_time}. 💰 Деньги любят счет! Твой подход идеален!",
            f"✅ ПРОРЫВНОЙ АНАЛИЗ {pair}: 1️⃣ Трейдер верно определил прорыв или отскок в {direction} движении. 2️⃣ Ключ: точное чтение {random_key_metrics} в условиях {random_condition} и {random_emotion}. 3️⃣ Развитие: искать аналогичные паттерны для увеличения частоты успешных сделок. 💡 Гениально! Ты опережаешь рынок.",
            f"✅ ФУНДАМЕНТАЛЬНЫЙ УСПЕХ {pair}: 1️⃣ {direction} сделка сработала благодаря учету **новостного фона** {random_time}. 2️⃣ Подтверждение: сочетание {random_analysis_style} и фундаментального анализа. 3️⃣ Рекомендация: продолжать держать руку на пульсе новостей. 📰 Глубокий анализ - залог стабильности!",
            f"✅ ПРАВИЛЬНЫЙ ИМПУЛЬС {pair}: 1️⃣ Трейдер умело вошел в {direction} движение на самом старте {random_time}. 2️⃣ Успех: подтверждение импульса по {random_key_metrics} и **рациональность** ({random_emotion}) в {random_condition}. 3️⃣ Совет: искать эти точки входа, они приносят самую большую прибыль. 🔥 Поймал волну! Идеальное использование импульса.",
            f"✅ КОНТР-ТРЕНД МАСТЕРСТВО {pair}: 1️⃣ Смелый и успешный {direction} вход против общего тренда {random_time}. 2️⃣ Факторы: **глубокий** {random_analysis_style} анализ, правильное определение зоны перекупленности/перепроданности. 3️⃣ План: применять эту тактику только при сильном подтверждении. 👑 Высокий класс! Ты видишь, что скрыто от других.",
            f"✅ ТЕХНИЧЕСКИЙ ОТСКОК {pair}: 1️⃣ Превосходный {direction} вход от ключевого уровня поддержки/сопротивления. 2️⃣ Причина: **дисциплина** ({random_emotion}) и точное совпадение с сигналом {random_key_metrics} в {random_condition}. 3️⃣ Развитие: заносить в дневник все сделки с отскоками для анализа. 📈 Техника в чистом виде! Браво.",
            f"✅ СКАЛЬПИНГ НА ВЫСОТЕ {pair}: 1️⃣ Четкий, быстрый и прибыльный {direction} вход {random_time}. 2️⃣ Ключ: {random_analysis_style} точность, минимальный стоп и быстрое решение в условиях {random_condition}. 3️⃣ Перспектива: продолжать тренировать скорость реакции и {random_emotion}. ⚡ Молниеносно! Каждая секунда принесла прибыль.",
            f"✅ ЖЕЛЕЗНАЯ ДИСЦИПЛИНА {pair}: 1️⃣ Успех не случаен — это результат четкого следования плану {direction}. 2️⃣ Подтверждение: **терпеливое** ожидание сигнала {random_key_metrics} и **отсутствие** спешки {random_time}. 3️⃣ Фокус: поддерживать эту дисциплину **всегда**. 💪 Твоя дисциплина дороже золота."
        ]
        return random.choice(success_analyses)
    
    elif is_loss:
        # 20 ВАРИАНТОВ УБЫТОЧНЫХ СДЕЛОК
        loss_analyses = [
            f"🔴 АНАЛИЗ УБЫТОЧНОЙ СДЕЛКИ {pair}: 1️⃣ Трейдер допустил ошибки при {direction} входе {random_time} и не дождался лучшего момента. 2️⃣ Психологические ошибки: эмоциональные решения, FOMO, жадность в условиях {random_condition}. 3️⃣ Рекомендации: найти лучшую точку входа, изменить подход, развивать {random_emotion}. 💪 Не сдавайся! Каждая сделка - это опыт! Продолжай торговать!",
            f"🔴 СТРОГИЙ АНАЛИЗ {pair}: 1️⃣ Трейдер не смог правильно использовать {direction} сигнал из-за поспешности {random_time}. 2️⃣ Ошибки: игнорирование сигналов рынка, неправильный тайминг, отсутствие {random_emotion} в {random_condition}. 3️⃣ Исправления: изучать технический анализ, тренировать терпение, следовать плану. 💪 Учись на ошибках! Каждая неудача приближает к успеху!",
            f"🔴 ПРОФЕССИОНАЛЬНЫЙ РАЗБОР {pair}: 1️⃣ Трейдер не учел рыночные условия при {direction} операции {random_time}. 2️⃣ Проблемы: недостаточный анализ, эмоциональные решения, нарушение правил в {random_condition}. 3️⃣ План действий: углубить знания, разработать четкую стратегию, развивать {random_emotion}. 💪 Помни: успех приходит к тем, кто учится! Не останавливайся!",
            f"🔴 КРИТИЧЕСКИЙ АНАЛИЗ {pair}: 1️⃣ Трейдер потерпел неудачу в {direction} сделке из-за отсутствия {random_emotion}. 2️⃣ Причины: поспешные решения {random_time}, игнорирование {random_condition}, нарушение дисциплины. 3️⃣ Исправления: изучать рынок, тренировать {random_emotion}, следовать стратегии. 💪 Каждая ошибка - шаг к мастерству! Продолжай развиваться!",
            f"🔴 ЭКСПЕРТНАЯ ОЦЕНКА {pair}: 1️⃣ Трейдер не справился с {direction} операцией в условиях {random_condition}. 2️⃣ Факторы неудачи: недостаток {random_emotion}, неправильный тайминг {random_time}, эмоциональность. 3️⃣ Путь к успеху: углубленное изучение, практика, развитие {random_emotion}, терпение. 💪 Мастерство приходит с опытом! Не сдавайся!",
            f"🔴 ЦЕННЫЙ УРОК {pair}: 1️⃣ Неверное решение по {direction} входу {random_time} - вероятно, не дождались подтверждения по {random_key_metrics}. 2️⃣ Психологический фактор: недостаток {random_emotion}, **импульсивность** в условиях {random_condition}. 3️⃣ Рекомендации: вернуться к базовым принципам, отработать **терпение**. 🤔 Анализируй! Убыток – это плата за обучение. Найди ошибку!",
            f"🔴 ДИСБАЛАНС {pair}: 1️⃣ Вероятно, трейдеру не хватило {random_emotion} для правильного {direction} решения. 2️⃣ Технические ошибки: вход **против тренда** или без подтверждения {random_key_metrics}. 3️⃣ Решение: пересмотреть риск-менеджмент и лимитировать эмоциональные сделки. 🚧 Внимание к деталям! Вернись к плану.",
            f"🔴 НАРУШЕНИЕ СИСТЕМЫ {pair}: 1️⃣ Сигнал был {direction}, но {random_analysis_style} анализ не был завершен {random_time}. 2️⃣ Факторы риска: **переторговка**, избыточная уверенность в {random_condition}. 3️⃣ Фокус: сосредоточиться на **одной** стратегии и исключить **жадность/страх**. 💡 Сделай паузу. Рынок никуда не убежит.",
            f"🔴 АНАЛИЗ РИСК-МЕНЕДЖМЕНТА {pair}: 1️⃣ Убыток по {pair} связан с неправильным выбором лота или стоп-лосса. 2️⃣ Проблемы: **Чрезмерный риск**, отсутствие {random_emotion} при закрытии позиции {random_time}. 3️⃣ Действие: пересчитать риски, использовать только **допустимый** процент от депозита. 🛡️ Защищай капитал! Это приоритет.",
            f"🔴 ПСИХОЛОГИЧЕСКИЙ РАЗБОР {pair}: 1️⃣ Поспешный {direction} вход, возможно, из-за **FOMO** (страха упущенной выгоды). 2️⃣ Ошибка: игнорирование четких сигналов {random_key_metrics} в условиях {random_condition}. 3️⃣ Исправление: развивать **самоконтроль** и всегда ждать **лучшей** цены. 🧘‍♂️ Будь хладнокровен! Эмоции - враг трейдера.",
            f"🔴 СИНДРОМ ПЕРЕТОРГОВКИ {pair}: 1️⃣ Сделка {direction} была открыта слишком рано или поздно, возможно, **после** серии убытков. 2️⃣ Психология: отсутствие {random_emotion}, попытка отыграться, игнорирование {random_key_metrics}. 3️⃣ Лечение: **принудительная пауза**, возврат к демо-счету или снижению лота. 🛑 Стоп! Отдохни и вернись к плану.",
            f"🔴 НЕДОСТАТОК ПОДТВЕРЖДЕНИЯ {pair}: 1️⃣ Вход в {direction} позицию был совершен **без** сигнала от {random_key_metrics} {random_time}. 2️⃣ Технический сбой: торговля на **слухах** или **догадках** в {random_condition}. 3️⃣ Исправление: Вводить сделку только при **полном** соответствии с чек-листом. 🧐 Торгуй фактами, а не надеждами!",
            f"🔴 ТОРГОВЛЯ ИЗ МЕСТИ {pair}: 1️⃣ {direction} сделка, мотивированная желанием 'наказать' рынок за предыдущие убытки. 2️⃣ Ошибка: полное отсутствие {random_emotion}, эмоциональный срыв в {random_condition}. 3️⃣ План: закрыть терминал. Изучить, почему стратегия не сработала, а не пытаться ее 'задавить'. 💔 Не мсти рынку! Он всегда прав.",
            f"🔴 ОТСУТСТВИЕ СТРАТЕГИИ {pair}: 1️⃣ Сделка {direction} похожа на случайный вход {random_time} без четкого плана. 2️⃣ Проблема: игнорирование {random_analysis_style} контекста и отсутствие {random_emotion}. 3️⃣ Действие: разработать **писаную** стратегию с правилами входа/выхода и стопами. 🗑️ Убери хаос! Торговля - это порядок.",
            f"🔴 ИГНОРИРОВАНИЕ РИСКА {pair}: 1️⃣ Убыток превысил допустимый лимит из-за неправильного объема или **отсутствия стопа**. 2️⃣ Фактор: чрезмерная самоуверенность или недостаток {random_emotion} в {random_condition}. 3️⃣ Немедленно: **Установить** правила РМ и **не нарушать** их под страхом потери счета. 🚨 Риск-менеджмент - твой телохранитель!",
            f"🔴 ОШИБКА УСРЕДНЕНИЯ {pair}: 1️⃣ Проблема в {direction} сделке — трейдер, вероятно, усреднял убыток вместо закрытия. 2️⃣ Риск: нарушение РМ, недостаток {random_emotion} при противодействии {random_condition}. 3️⃣ Правило: **НИКОГДА** не усреднять убытки. Сработал стоп — жди нового сигнала. 🛑 Усреднение убивает счет. Соблюдай стоп.",
            f"🔴 ЛОЖНЫЙ ПРОБОЙ {pair}: 1️⃣ {direction} вход оказался ловушкой {random_time} — цена вернулась в диапазон. 2️⃣ Технический недочет: не дождались **закрепления** цены, не хватило {random_emotion}. 3️⃣ Исправление: использовать подтверждающий сигнал {random_key_metrics} для исключения ложных пробоев. 📉 Не попадись в ловушку! Жди подтверждения.",
            f"🔴 ИГНОРИРОВАНИЕ НОВОСТЕЙ {pair}: 1️⃣ Убыток связан с **неожиданной** реакцией рынка на новости в {random_time}. 2️⃣ Ошибка: торговля в {random_condition} без **фундаментального** {random_analysis_style} анализа. 3️⃣ План: проверять экономический календарь перед входом и избегать торговли перед 'красными папками'. 📰 Новости двигают рынок. Будь готов.",
            f"🔴 НЕПРАВИЛЬНАЯ КОРРЕКЦИЯ {pair}: 1️⃣ Трейдер принял коррекцию за разворот и открыл {direction} позицию. 2️⃣ Недочет: игнорирование старшего таймфрейма, недостаток {random_emotion} в условиях {random_condition}. 3️⃣ Решение: применять {random_analysis_style} анализ для определения **глобального** тренда. 🔄 Тренды сильны! Не путай коррекцию с разворотом.",
            f"🔴 ПОДДАЛСЯ ЧУВСТВАМ {pair}: 1️⃣ Убыточная {direction} сделка — результат **чистых эмоций**, а не расчета. 2️⃣ Психология: **жадность** или **страх** привели к нарушению {random_key_metrics} сигнала. 3️⃣ Совет: не торговать, пока не вернется **рациональность** ({random_emotion}). 💔 Эмоции - враг успеха. Торгуй с холодной головой."
        ]
        return random.choice(loss_analyses)
    
    else:
        # 10 ВАРИАНТОВ ОБЩИХ АНАЛИЗОВ
        general_analyses = [
            f"📊 АНАЛИЗ СИГНАЛА {pair}: 1️⃣ Проверьте технические индикаторы и рыночные условия для {direction}. 2️⃣ Убедитесь в правильности выбора направления и времени входа. 3️⃣ Следуйте стратегии управления рисками и не нарушайте дисциплину. 💪 Успешной торговли!",
            f"📈 ПРОФЕССИОНАЛЬНАЯ ОЦЕНКА {pair}: 1️⃣ Изучите график и определите тренд перед {direction} входом. 2️⃣ Проверьте объемы, волатильность и фундаментальные факторы. 3️⃣ Планируйте сделку заранее и строго следуйте плану. 💪 Торгуй умно и прибыльно!",
            f"🎯 СТРАТЕГИЧЕСКИЙ АНАЛИЗ {pair}: 1️⃣ Оцените рыночную ситуацию и выберите оптимальный {direction} момент. 2️⃣ Используйте стоп-лоссы и тейк-профиты для защиты капитала. 3️⃣ Ведите дневник сделок и анализируйте результаты. 💪 Развивайся и зарабатывай!",
            f"📝 ЧЕК-ЛИСТ ПЕРЕД ВХОДОМ {pair}: 1️⃣ Подтвердите {direction} сигнал индикатором {random_key_metrics}. 2️⃣ Оцените текущий {random_analysis_style} контекст и {random_condition}. 3️⃣ Убедитесь, что ваш {random_emotion} находится под контролем. 🚀 Удачи! Пусть риск будет оправдан.",
